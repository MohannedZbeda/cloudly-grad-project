<template>
    <div>
        ...
        <v-data-table
            ...
            :no-data-text="
                $translate(`There's no variants..`, 'لا يوجد منتجات..')
            "
        >
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <v-card>
                        <v-card-title class="text-h5"
                            >{{
                                $translate(
                                    `${variant.en_name}'s Attributes`,
                                    `خصائص ${variant.ar_name}`
                                )
                            }}<br />
                        </v-card-title>
                        <v-card-text dir="rtl">
                ...
                <v-dialog v-model="delete_discount_dialog" max-width="500px">
                    <v-card>
                        ...
                        <v-card-text dir="rtl">
                            {{
                                $translate(
                                    `Are you sure you want to delete ${variant.en_name}'s Discount ?`,
                                    `  هل أنت متأكد من أنك تريد حذف تخفيض المنتج ${variant.ar_name} `
                                )
                            }}
                        </v-card-text>
                        <v-card-actions>
                ...
                <v-dialog v-model="add_vouchers_dialog" max-width="500px">
                    <v-card>
                        <v-card-title class="text-h5"
                            >{{
                                $translate(
                                    `Add Vouchers to ${variant.en_name}`,
                                    `إضافة هدايا ل ${variant.ar_name}`
                                )
                            }}<br />
                        </v-card-title>
                        <v-card-text dir="rtl">
                            <b>{{
                                $translate(
                                    "Gifs are codes to be given to chosen users, When used, The codes give the chosen variant",
                ...
                <v-dialog v-model="add_discount_dialog" max-width="500px">
                    <v-card>
                        <v-card-title class="text-h5"
                            >{{
                                $translate(
                                    `Add Discount to ${variant.en_name}`,
                                    `إضافة تخفيض ل ${variant.ar_name}`
                                )
                            }}<br />
                        </v-card-title>
                        <v-card-text dir="rtl">
...
export default {
    ...
    methods: {
        ...
        removeDiscount() {
            ...
            this.sending = true;
            Variantservice.RemoveDiscount(payload)
                .then(response => {
                    this.variants = response.data.variants;
                })
                .finally(() => {
        ...
        addDiscount() {
            ...
            this.sending = true;
            Variantservice.AddDiscount(payload)
                .then(response => {
                    this.variants = response.data.variants;
                    this.$swal(
                        this.$translate(
        ...
        addVouchers() {
            ...
            Variantservice.AddVouchers(payload)
                .then(response => {
                    this.variants = response.data.variants;
                    this.$swal(
                        this.$translate(
